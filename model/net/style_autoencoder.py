...
class StyleTransfer(keras.Model):
    ...
    def __init__(self, style_weight=10, content_weight=1, **kwargs):
    ...
    def preprocess_input(self, content_img:tf.Tensor, style_img:tf.Tensor, resize=None):
        # Unscaled decentered RGB (uint8) -> scaled centered BGR (float32)
    ...
    def postprocess_output(self, reconstructed_image:tf.Tensor, output_shape):
        # Scaled centered BGR (float32) -> unscaled decentered RGB (uint8)
    ...
    # Inference
    def call(self, content:tf.Tensor, style:tf.Tensor, alpha=1, save_content_colors=False):
